...
The app as a Vite + React 19 + TypeScript project with a small design system.
It was rebuilt from an exported design prototype, screen for screen; that
export is kept in `template-backup/` as a reference until it is removed, and
its own README explains how it works.

```sh
bun install
bun run dev        # http://localhost:5173 (the prototype backup runs on 5174)
